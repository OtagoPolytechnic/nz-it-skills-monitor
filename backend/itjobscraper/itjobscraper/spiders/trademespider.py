...
class TrademespiderSpider(scrapy.Spider):
    ...
    def parse_job_page(self, response):
        ...
        job_item['location'] = response.xpath("/html/body/tm-root/div[1]/main/div/ng-component/div/div[4]/div/div[1]/div/tm-jobs-listing-body/tm-key-details-rack/tg-rack/tg-rack-item[1]/div/div/tg-rack-item-secondary/text()").get()                     
        ...
        description_parts = response.css('.tm-jobs-listing-body__item-content ::text').getall()
        job_item['description'] = ' '.join([part.strip() for part in description_parts if part.strip()])
        job_item['source'] = "trademe"
        yield job_item