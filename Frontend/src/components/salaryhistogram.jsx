...
export default function SalaryHistogram() {
  ...
  const data = useMemo(() => {
    const map = Object.fromEntries(
      (buckets || []).map((b) => [b.band, b.count])
    );
    return ORDER.map((band) => ({ band, count: Number(map?.[band] || 0) }));
  }, [buckets]);

  if (loading)
    return <div style={{ height: 360 }}>Loading salary distribution…</div>;
  if (err)
    return (
      <div style={{ height: 360, color: "crimson" }}>
        Failed to load salary distribution.
      </div>
    );
    ...
    <div style={{ height: 360 }}>
      <div className="card-title" style={{ marginBottom: 8 }}>
        ...
        <DownloadCSVButton
          ...
          filename="salary_distribution.csv"
          rows={data}
          columns={[
            ["Band", "band"],
            ["Jobs", "count"],
